...
namespace DAL
{
    public class OrderDao : BaseDao
    {
        ...
        {
            string query = @"
        SELECT O.Order_Id, O.Employee_Id, O.Table_Id, O.Status, O.Placed_Time, OI.Menu_Id, 
               OI.Quantity, OI.Note, OI.Unit_Price,
               M.Name, M.Price, M.Is_Alcoholic
        FROM Orders AS O
        INNER JOIN Order_Item AS OI ON O.Order_Id = OI.Order_Id
        INNER JOIN Menu_Item AS M ON OI.Menu_Id = M.Menu_Id
            ...
            while (reader.Read())
            {
                if (order == null)
                {
                    order = new Order
                    {
                        ...
                        Employee = authDao.GetEmployeeById((int)reader["Employee_Id"]),
                        Table = tableDao.GetTableByOrderId((int)reader["Order_Id"]),
                        Status = Enum.Parse<OrderStatus>(reader["Status"].ToString()),
                        PlacedTime = (DateTime)reader["Placed_Time"],
                ...
                MenuItem menuItem = new MenuItem
                {
                    ...
                    IsAlcoholic = (bool)reader["Is_Alcoholic"]
                };
